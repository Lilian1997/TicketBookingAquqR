    ...
    if (err) {
      ...
      transporter
        .sendMail({
          ...
          html: `${req.body.contactName} 您好，我們已經收到您送出於 ${req.body.selectedDate} 場次 ${req.body.selectedSession}:00 的團體預約申請，預約是成人票${req.body.adultTicket}張、學生票${req.body.studentTicket}張、孩童票${req.body.childTicket}張、博愛票${req.body.charityTicket}張，我們會於3-5個工作天內回覆，感謝您！`,
        })
        .then((info) => {
          console.log({ info });
        })
...
app.post("/bookingInquiry", async function (req, res) {
  ...
  await conn.query(sql, data, async function (err, orderRows) {
    ...
    if (err) {
      ...
      if (orderRows[0]) {
        const ticketSql =
          "select * from ticket where order_id = ? and (valid = ? or valid=?) ORDER BY FIELD(SUBSTRING(ticket_type , 1 , 1), 'A', 'S','K','E')";
