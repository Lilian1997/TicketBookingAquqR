...
function Cart() {
    ...
    await axios.get(url)
      .then(function (response) {
        ...
        let DateData = response.data.data.map(function (element) {
          return (new Date(element.date).getFullYear() + '-' + (new Date(element.date).getMonth() + 1) + '-' + (new Date(element.date).getDate()))
  ...
  // 調查場次是否available
  let [disabledSession, setdisabledSession] = useState({ nine: 0, twelve: 0, fifteen: 0 });
  const sessionAvailable = async (e) => {
    ...
    let Session = [];
    await axios.post(posturl, { date: e })
      .then(function (response) {
  ...
  const AvailableCheck = async (e) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      let Total = selectedTickets.reduce((acc, element) => acc + element.count, 0);
  ...
  const GenerateTickets = (element) => {
    ...
    for (let i = 1; i <= element.count; i++) {
      ...
      let selectedSession = document.querySelector('[name=ChooseSession]:checked').value;
  ...
  return (
    <Fragment>
        ...
        <div className="TicketContainer  container">
          ...
          <div className="d-flex justify-content-center">
            <div className="shoppingSOPbox">
              <div
                className="bg-warning text-dark shoppingSOP"
              >
                選購
                <br />
                票券
          ...
          <div className="Cart_date row">
            <div className="col-md-4 col-12 d-flex justify-content-center">
              <div className="">
                <div className="fw-semibold mb-3 text-center Cart_date_title">選擇日期</div>

                {DateSelect.length > 0 && (
                  <select name="DateYYMMDD"
                    id="DateYYMMDD"
...

          {
            ...
            (selectedSessionOption === "" || document.getElementById('DateYYMMDD').value === "---請選擇---") ? null : (
              <div>
                <div className="fw-semibold mb-3 text-center Cart_date_title">請先勾選票種再選擇數量</div>
                  ...
                  <div id="InCartInfo">
                    {selectedTickets.length === 0 ? (
                      <div id="NothingInCart">
                        <div className="admissionTimeBox border-0 mb-0 fs-5">尚未選購任何票券</div>
                      </div>
                    ) : (
      ...
      className="modalAlert">
        ...
        <Modal.Body className="text-center text-dark">已刪除此票券！</Modal.Body>
        <Modal.Footer className="justify-content-center">
